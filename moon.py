...
KAGGLE_NOTEBOOK_BASE_LUNAR = "/kaggle/input/space-apps-2024-seismic-detection/space_apps_2024_seismic_detection/data/lunar/training/data" 
...
def guess_data_root(user_root: Optional[str], force_download: bool, dataset: str) -> str:
    """Resolve root containing lunar .mseed files within combined dataset.

    Priority:
      1. user_root if provided and exists
      2. Kaggle lunar default path (if exists)
      3. Download via kagglehub
    """
    if user_root and os.path.isdir(user_root):
        return user_root
    if os.path.isdir(KAGGLE_NOTEBOOK_BASE_LUNAR):
        return KAGGLE_NOTEBOOK_BASE_LUNAR
    if force_download or True:
        return download_dataset(dataset)


def collect_mseed_files(root: str, limit: Optional[int] = None, planet: str = "lunar") -> List[str]:
    """Collect MiniSEED files for the specified planet (default lunar) from combined dataset."""
    planet_lower = planet.lower() if planet else ""
    matches: List[str] = []
    for dirpath, _dirnames, filenames in os.walk(root):
        rel = dirpath.replace("\\", "/").lower()
        if planet_lower and planet_lower not in rel:
            continue
        for f in filenames:
...
def parse_args():
    p = argparse.ArgumentParser(description="Lunar MiniSEED exploration utility (filtered from combined dataset)")
...
def main():
    ...
    files = collect_mseed_files(root, limit=args.limit if args.list else None, planet="lunar")
    ...
    if not files or (args.limit and len(files) < args.index + 1):
        files = collect_mseed_files(root, limit=None, planet="lunar")
    ...
    # ML detection after optional plotting
